...
| Semantic extraction (LLM) | **Done (markdown + PDF + images, 5 backends)** | `internal/llm` provider-agnostic interface plus five implementations: `anthropic` (Claude), `openai` (GPT-4o-mini), `gemini` (Flash, key in query param), `ollama` (local, no API key), `kimi` (Moonshot, OpenAI-API-compatible). All five absorb vision via `llm.Request.Images` — different wire formats (Anthropic source-object, OpenAI/Kimi image_url data URI, Gemini inlineData, Ollama base64 array) hidden behind one interface. `semantic.Extract` for text; `semantic.ExtractImage` for visual content with a vision-specific prompt. Pipeline routes documents raw, PDFs via `extract.PDFPlainText`, images via `http.DetectContentType` (PNG/JPEG/WebP/GIF supported). Parallel fan-out with 4 workers. |
...
| Feature | Upstream Location | Description |
|---------|-------------------|-------------|
| URL fetching with type detection | **Partial (PDF + HTML + tweet/OG; arXiv URL rewrite)** | `Ingest` detects PDF responses via `%PDF-` magic bytes plus URL-suffix fallback. arXiv `abs/<id>` URLs are rewritten to `pdf/<id>.pdf` before fetching. HTML responses now have OpenGraph + Twitter-Card meta tags (`og:title`/`og:description`/`og:image`/`og:site_name`, with `twitter:*` fallback) extracted and prepended as a quoted metadata block — preserves title/description signal even when JS-gated body content is missing (typical of X/Twitter status pages). `IsTweetURL` recognizes twitter.com / x.com / mobile.twitter.com status URLs and labels the sidecar frontmatter with `kind: tweet`. GitHub/YouTube detection deferred. |
