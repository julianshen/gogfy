...
| Semantic extraction (LLM) | **Done (markdown + PDF, 4 backends)** | `internal/llm` provider-agnostic interface plus four implementations: `anthropic` (Claude), `openai` (GPT-4o-mini), `gemini` (Flash, key in query param), `ollama` (local, no API key). `internal/semantic.Extract` prompts for entities + relations, returns schema-shaped nodes/edges (Confidence=INFERRED). `--semantic --backend {anthropic\|openai\|gemini\|ollama}`. Document files routed raw; PDF files run through `extract.PDFPlainText` first (panic-tolerant). Parallel fan-out with 4 workers. Images still deferred (needs vision capability). |
