...
| Semantic extraction (LLM) | **Done (v1, markdown, 3 backends)** | `internal/llm` provider-agnostic interface plus three implementations: `anthropic` (Claude), `openai` (GPT-4o-mini), `ollama` (local, no API key, honors `OLLAMA_HOST`). `internal/semantic.Extract` prompts for entities + relations, returns schema-shaped nodes/edges (Confidence=INFERRED). `--semantic --backend {anthropic\|openai\|ollama}`; doc files only in v1. Parallel fan-out with 4 workers. |
