...
| Semantic extraction (LLM) | **Done (markdown + PDF + images, 4 backends)** | `internal/llm` provider-agnostic interface plus four implementations: `anthropic` (Claude), `openai` (GPT-4o-mini), `gemini` (Flash, key in query param), `ollama` (local, no API key). All four absorb vision via `llm.Request.Images` — different wire formats (Anthropic source-object, OpenAI image_url data URI, Gemini inlineData, Ollama base64 array) hidden behind one interface. `semantic.Extract` for text; `semantic.ExtractImage` for visual content with a vision-specific prompt. Pipeline routes documents raw, PDFs via `extract.PDFPlainText`, images via `http.DetectContentType` (PNG/JPEG/WebP/GIF supported). Parallel fan-out with 4 workers. |
