...
## Supported (29 code + 8 document formats)
...
| Erlang | `.erl` `.hrl` | `github.com/julianshen/tree-sitter-erlang` (fork of `WhatsApp/tree-sitter-erlang`; upstream's `binding.go` omits the external-scanner `#include` and declares a module path that doesn't match the repo location, so the fork fixes both) |
...
**Code grammars** — each needs a fork repo under `julianshen/tree-sitter-<lang>`. Swift, R, and Erlang are the precedents:
