...
func TestDispatchComboUninstallClaude(t *testing.T) {
	ws := t.TempDir()
	if err := os.MkdirAll(filepath.Join(ws, ".git", "hooks"), 0755); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(ws, "CLAUDE.md")); !os.IsNotExist(err) {
		t.Fatal("CLAUDE.md should be removed (snippet was the only content)")
	}
	if _, err := os.Stat(filepath.Join(ws, ".git", "hooks", "post-commit")); !os.IsNotExist(err) {
		t.Fatal("post-commit should be removed")
	}
}
