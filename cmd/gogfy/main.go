...
func comboCommand(platform, verb string, args []string, stderr io.Writer) error {
	...
	if verb == "uninstall" {
		// Best-effort: attempt every step, aggregate failures.
		var errs []string
		if inst, err := installer.For(platform); err == nil {
			if err := inst.Uninstall(abs); err != nil {
				errs = append(errs, fmt.Sprintf("mcp: %v", err))
			}
		}
		if err := installer.UninstallSnippet(docsFile); err != nil {
			errs = append(errs, fmt.Sprintf("snippet: %v", err))
		}
		if err := githook.Uninstall(abs); err != nil {
			errs = append(errs, fmt.Sprintf("hook: %v", err))
		}
		if len(errs) > 0 {
			return fmt.Errorf("%s uninstall: %s", platform, strings.Join(errs, "; "))
		}
		fmt.Fprintf(stderr, "gogfy: %s uninstall complete\n", platform)
		return nil
	}
